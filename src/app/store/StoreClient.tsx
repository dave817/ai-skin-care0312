...
import { mainCategories, type Product } from "@/data/products";
...
const categoryTabs = [
  ...
  ...mainCategories.filter((c) =>
    ["skincare", "body-hair", "makeup", "tools", "health"].includes(c.id)
  ),
];
...
function StoreInner({ allProducts }: StoreClientProps) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case "newest":
        products = [...products]
          ...
          .concat([...products].filter((p) => !p.tags.includes("new")));
        break;
      default:
    ...
    return products;
  }, [allProducts, activeCategory, activeSubcategory, sortBy, searchQuery]);
    ...
    <main className="min-h-screen bg-bg-primary pb-20">
      <div className="container-main pt-6 pb-4">
        ...
        <p className="text-sm text-text-muted mt-1">
          ...
          {searchQuery && (
            <span className="ml-2 text-accent-rose">
              · 搜尋「{searchQuery}」
            </span>
          )}
        </p>
            ...
            <div className="flex gap-3 overflow-x-auto pb-3 scrollbar-hide -mx-4 px-4">
              {subcategories.map((sub) => (
                <button
                  ...
                  className={`flex-shrink-0 text-sm transition-all duration-200 pb-1 border-b-2 ${
...
export default function StoreClient({ allProducts }: StoreClientProps) {
  return (
    <Suspense
      fallback={
    ...
    >
      <StoreInner allProducts={allProducts} />
    </Suspense>
  );
}
