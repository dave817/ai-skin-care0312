...
import { listProducts, upsertProduct, seedProductsToRedis } from "@/lib/db/products-repo";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (body?.action === "seed") {
      const result = await seedProductsToRedis();
    ...
    /* Sanitize HTML in description to prevent stored XSS */
    const sanitized = {
      ...parsed.data,
